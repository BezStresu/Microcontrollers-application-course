...
bool USART_PutChar(char c){
	__disable_irq();
	bool status = RingBuffer_PutChar(&USART_RingBuffer_Tx, c);
	__enable_irq();
	if(status) __USART_ENABLE_IT(&UartHandle, USART_IT_TXE);
	return status;
}
...
size_t USART_WriteData(const void *data, size_t dataSize){
	size_t i = 0;
	while(i < dataSize){
		bool status = USART_PutChar(((const char*)data)[i]);
		if (!status) break;
		i++;
	}
	return i;
...
bool USART_GetChar(char *c){
	__disable_irq();
	bool status = RingBuffer_GetChar(&USART_RingBuffer_Rx, c);
	__enable_irq();
	return status;
}
...
size_t USART_ReadData(char *data, size_t maxSize){
	size_t i = 0;
	while(i < maxSize){
		bool status = USART_GetChar(data + i);
		//bool status = RingBuffer_PutChar(&USART_RingBuffer_Rx, ((const char*)data)[i]);
		if (!status) break;
		i++;
	}
	return i;
